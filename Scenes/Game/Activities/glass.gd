	...
	if( event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT):
		if event.pressed:
			...
			for ds in drop_spots:
				if ds.monitoring and get_parent().selected_soda == get_parent().SodaTypes.NONE and ds.get_overlapping_areas() and ds.get_overlapping_areas().has(self.get_node("../DrinkArea")): #this is if we want snapping
